...
class TabularDataSource < DataSource
  ...
  def splitHeaderLineFromContent(content)
    # We assume that the first line is the header line and that it has no line breaks.
  ...
  def removeSuperfluousLineBreaksFromTabularFile(separator, fieldCount, content)
    #TODO
  end
